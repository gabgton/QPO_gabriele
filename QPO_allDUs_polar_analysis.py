...
sys.path.insert(0,'/Users/gabri/OneDrive/Fisica/TESI/QPO_python/QPO_python_OLD_version/analisi')
from polarization_plots import *
# round_to_significant_figures rimosso da qui
...
outfile = f'/Users/gabri/OneDrive/Fisica/TESI/QPO_python/QPO_python_OLD_version/Obs1/polarization_ALLDU_new_{seg_size}s_{method}_v.txt'
file = open(outfile, 'w')
...
for thr in THR:
    ...
    for i in range(len(selected_title)):
        ...
        print(f'QN = {np.round(qn[i]*100.,1)} +- {np.round(qn_err[i]*100.,0)} %')
        print(f'UN = {np.round(un[i]*100.,1)} +- {np.round(un_err[i]*100.,0)} %')
        print(f'PD = {np.round(pd[i]*100.,1)} +- {np.round(pd_err[i]*100.,0)} %')
        print(f'PA = {np.round(pa[i]*1.,2)} +- {np.round(pa_err[i]*1.,3)} °')
        print(f'MDP = {np.round(mdp[i]*100.,3)} %')
        print(f'{not_selected_title[i]} events\n')
        print(f'QN = {np.round(qn_n[i]*100.,1)} +- {np.round(qn_err_n[i]*100.,0)} %')
        print(f'UN = {np.round(un_n[i]*100.,1)} +- {np.round(un_err_n[i]*100.,0)} %')
        print(f'PD = {np.round(pd_n[i]*100.,1)} +- {np.round(pd_err_n[i]*100.,0)} %')
        print(f'PA = {np.round(pa_n[i]*1.,2)} +- {np.round(pa_err_n[i]*1.,3)} °')
        print(f'MDP = {np.round(mdp_n[i]*100.,3)} %')
        print(f'')
        if save_out_file==True:
            file.write(f'{selected_title[i]} events\n')
            file.write(f'QN = {np.round(qn[i]*100.,1)} +- {np.round(qn_err[i]*100.,0)} %\n')
            file.write(f'UN = {np.round(un[i]*100.,1)} +- {np.round(un_err[i]*100.,0)} %\n')
            file.write(f'PD = {np.round(pd[i]*100.,1)} +- {np.round(pd_err[i]*100.,0)} %\n')
            file.write(f'PA = {np.round(pa[i]*1.,2)} +- {np.round(pa_err[i]*1.,3)} °\n')
            file.write(f'MDP = {np.round(mdp[i]*100.,3)} %\n')
            file.write(f'{not_selected_title[i]} events\n')
            file.write(f'QN = {np.round(qn_n[i]*100.,1)} +- {np.round(qn_err_n[i]*100.,0)} %\n')
            file.write(f'UN = {np.round(un_n[i]*100.,1)} +- {np.round(un_err_n[i]*100.,0)} %\n')
            file.write(f'PD = {np.round(pd_n[i]*100.,1)} +- {np.round(pd_err_n[i]*100.,0)} %\n')
            file.write(f'PA = {np.round(pa_n[i]*1.,2)} +- {np.round(pa_err_n[i]*1.,3)} °\n')
...
if save_table:
    ...
    df.to_csv(f'/Users/gabri/OneDrive/Fisica/TESI/QPO_python/QPO_python_OLD_version/Obs1/ALL_DU_merged_{method}_{seg_size}s_v.csv', index=False)
# plt.show()